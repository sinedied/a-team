---
name: skill-builder
description: Create, refine, or retire a skill for this project. Use when the user has a repeatable workflow worth capturing as a reusable skill, wants to improve or fix an existing skill, or wants to remove one. This skill is the authority on the skill format and authoring best practices. Do not use for one-off tasks the agent already handles well.
---
...
Markdown body), plus optional `references/`, `tools/`, `assets/`.
...
## Tools / scripts (only when needed)

Add a script (under `tools/`) only for work that's deterministic, fragile, repetitive, or
token-heavy (parsing, conversion, validation) — something reasoning alone can't do reliably.
A script must be non-interactive (input via flags/stdin), print structured stdout with
diagnostics on stderr, give helpful errors and meaningful exit codes, be idempotent, require
`--confirm` for destructive actions, and pin versions. Prefer no dependencies (Python
stdlib / Node built-ins); markdown is the default, scripts are the exception.
...
4. Scaffold `.github/skills/<name>/SKILL.md`; add `tools/`/`references/`/`assets/` only if needed.
...
6. Commit with a conventional-commit message (e.g. `feat: add <name> skill`).
...
3. If you touched the description, re-run the triggering sanity-check. Commit.

## Retiring a skill

1. Confirm the skill is no longer needed.
2. Delete its folder. Commit.

## Gotchas

- Commit changes with a conventional-commit message (`feat:`/`fix:`/`refactor:`/`docs:`/…).
- Portability: target the open `SKILL.md` standard and avoid vendor-specific features so
  skills work across any agent. Note real environment needs in a `compatibility` field when
  relevant.
- The description is load-bearing — a skill that never triggers (or misfires constantly) is
  worse than no skill. Always run the should-load / should-not-load sanity check.
