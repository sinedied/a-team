...
- Read `devdocs/memory/decisions.md`, `devdocs/specs/roadmap.md` (if present), `README.md` to understand audience, domain, and any tonal cues already implied.
...
- `devdocs/brand/brand-book.html` — visual reference of colors, typography, gradients, pairings
- `devdocs/brand/ui-kit.html` — reference implementation of base components (buttons, labels, cards, trust bars, badges, footer)
- `devdocs/brand/demo.html` — sample implementation of a key page (e.g. homepage) showcasing the identity in action with fake data
...
Append the meaningful brand decisions (palette rationale, typography choice, aesthetic direction, references that informed the direction) to `devdocs/memory/decisions.md` using the project's decision format. The detailed values live in `DESIGN.md`; the *why* lives in `devdocs/memory/decisions.md`.
...
5. Append the change rationale to `devdocs/memory/decisions.md`.
6. If `devdocs/brand/brand-book.html` or `devdocs/brand/ui-kit.html` exist, regenerate them to match.
