...
> **Run as the art-director agent.** If you are not the `art-director` (Frankie), delegate the entire brand work to it via the task/agent tool before proceeding. The art-director owns `DESIGN.md` end-to-end; running brand work from another agent splits ownership and risks inconsistent identity decisions.
