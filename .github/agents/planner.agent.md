...
3. **Design** — Propose architecture and break the work into ordered subtasks. Each subtask must have a clear definition of done. Identify constraints and dependencies. If the feature has UI/UX aspects, first check `memory/brand.md`: if `Status: Defined` is `no`, stop and report back so the orchestrator can invoke the `design-director` (Walter) before planning continues. Then delegate to the `designer` agent to produce the Design section of the spec — pointing them at `memory/brand.md` as the visual contract — before finalizing. For features with user-facing behavior, include subtasks for e2e/integration tests covering the critical paths.
