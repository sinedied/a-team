...
1. **Understand scope** — Read the relevant spec in `devdocs/specs/` to understand what was built and its acceptance criteria. Check `devdocs/memory/decisions.md` for relevant context. Read the QA log in `devdocs/qa/<feature>_log.md` if it exists — re-run previously tested scenarios to catch regressions.
...
10. **Update QA log** — After reporting, write or update `devdocs/qa/<feature>_log.md` with all scenarios tested, edge cases probed, and issues found. This log persists across QA sessions so future runs don't start from scratch. Create the `devdocs/qa/` directory if it doesn't exist.
...
- DO NOT modify any code. Report issues, don't fix them. The only file you write to is `devdocs/qa/<feature>_log.md`.
